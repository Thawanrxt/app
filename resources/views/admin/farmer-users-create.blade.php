...
<div class="page-head">
  <div class="page-title">
    ...
    <div>
      ...
      <p class="muted">บันทึกข้อมูลเกษตรกร โปรไฟล์ และแปลงหลักลงฐานข้อมูลจริง</p>
    </div>
  </div>
</div>
...
@if (session('success'))
  <div class="card" style="margin-top: 16px; border-color: #86efac; background: #f0fdf4;">
    ...
    @if (!empty($createdPlotFarmId))
      <div style="margin-top: 8px;">รหัสแปลงที่สร้างอัตโนมัติ: <strong>{{ $createdPlotFarmId }}</strong></div>
    @endif
  </div>
@endif

<form method="POST" action="/admin/farmer-users/create">
  @csrf
  <div class="card" style="margin-top: 16px; border-color: #bfdbfe; background: #eff6ff;">
    <strong>ข้อมูลสำหรับเข้าแอพ</strong>
    <div class="muted" style="margin-top: 8px;">
      ผู้ใช้สามารถล็อกอินด้วย <strong>ชื่อผู้ใช้</strong> หรือ <strong>รหัสทะเบียนเกษตรกร</strong> ได้ โดยใช้รหัสผ่านเดียวกัน
    </div>
  </div>
  <div class="card" style="margin-top: 16px;">
    ...
    <div class="form-grid">
      <label>ชื่อ-นามสกุล
        <input class="input" name="name" type="text" placeholder="กรอกชื่อเกษตรกร" value="{{ old('name') }}">
      </label>
      <label>เลขบัตรประชาชน
        <input class="input" name="citizen_id" type="text" inputmode="numeric" autocomplete="off" maxlength="17" placeholder="กรอกเลขบัตรประชาชน" value="{{ old('citizen_id') }}" data-format="citizen-id" oninput="window.AdminInputMasks && window.AdminInputMasks.apply(this, 'citizen-id')" onblur="window.AdminInputMasks && window.AdminInputMasks.apply(this, 'citizen-id')">
        <span class="input-error-text" data-input-error-for="citizen_id">ต้องใส่เป็นตัวเลขเท่านั้น</span>
      </label>
      <label>เบอร์โทรศัพท์
        <input class="input" name="phone" type="text" inputmode="numeric" autocomplete="off" maxlength="12" placeholder="กรอกเบอร์โทรศัพท์" value="{{ old('phone') }}" data-format="phone" oninput="window.AdminInputMasks && window.AdminInputMasks.apply(this, 'phone')" onblur="window.AdminInputMasks && window.AdminInputMasks.apply(this, 'phone')">
        <span class="input-error-text" data-input-error-for="phone">ต้องใส่เป็นตัวเลขเท่านั้น</span>
      </label>
      <label>วันเดือนปีเกิด (วัน/เเดือน/พ.ศ.)
        <input class="input" name="birth_date" type="date" value="{{ old('birth_date') }}" data-buddhist-date>
      </label>
      <label class="plot-code-label">รหัสแปลง
        <input
          class="input"
          type="text"
          value="{{ old('plot_farm_id', $createdPlotFarmId ?? '') }}"
          placeholder="ระบบจะสร้างให้อัตโนมัติหลังบันทึกข้อมูล"
          readonly
          disabled
        >
      </label>
  ...
  <div class="card" style="margin-top: 16px;">
    ...
    <div class="form-grid">
      <label>ที่อยู่เลขที่/หมู่/ซอย/ถนน
        <input class="input" name="address_line" type="text" placeholder="บ้านเลขที่, หมู่, ซอย, ถนน" value="{{ old('address_line') }}">
      </label>
      <label>จังหวัด
        <select class="input" name="province" id="province-select" data-selected="{{ old('province') }}">
          <option value="">{{ old('province') ? 'กำลังโหลดข้อมูล...' : 'เลือกจังหวัด' }}</option>
        </select>
      </label>
      <label>อำเภอ/เขต
        <select class="input" name="district" id="district-select" data-selected="{{ old('district') }}" disabled>
          <option value="">{{ old('district') ? 'กำลังโหลดข้อมูล...' : 'เลือกอำเภอ/เขต' }}</option>
        </select>
      </label>
      <label>ตำบล/แขวง
        <select class="input" name="subdistrict" id="subdistrict-select" data-selected="{{ old('subdistrict') }}" disabled>
          <option value="">{{ old('subdistrict') ? 'กำลังโหลดข้อมูล...' : 'เลือกตำบล/แขวง' }}</option>
        </select>
      </label>
    </div>
  </div>
  ...
  <div class="card" style="margin-top: 16px;">
    ...
    <div class="form-grid">
      ...
      <label>วันที่ขึ้นทะเบียน
        <input class="input" name="registered_at" type="date" value="{{ old('registered_at') }}" data-buddhist-date>
      </label>
      <label>จังหวัดที่ขึ้นทะเบียน
        <select class="input" name="registered_province" id="register-province-select" data-selected="{{ old('registered_province') }}">
          <option value="">{{ old('registered_province') ? 'กำลังโหลดข้อมูล...' : 'เลือกจังหวัดที่ขึ้นทะเบียน' }}</option>
        </select>
      </label>
    </div>
  </div>
    ...
    <div class="form-grid" style="margin-top: 12px;">
      <label>ผู้ดูแลหลัก
        <select class="input" name="assigned_admin_user_id">
          ...
          @foreach (($assignableAdminOptions ?? []) as $adminOption)
            <option value="{{ $adminOption['id'] }}" @selected(old('assigned_admin_user_id') === $adminOption['id'])>{{ $adminOption['label'] }}</option>
          @endforeach
        </select>
      </label>
      <label>ผู้ดูแลร่วม
            ...
            @foreach (($assignableAdminOptions ?? []) as $adminOption)
              <label class="multi-select-option">
                <input
                  ...
                  @checked(collect(old('secondary_admin_user_ids', []))->contains($adminOption['id']))
                >
      ...
      <label>ประเภทการดูแล<span class="required-star">*</span>
        <select class="input" name="assignment_type">
          ...
          <option value="AREA" @selected(old('assignment_type') === 'AREA')>ดูแลตามพื้นที่</option>
          <option value="INDIVIDUAL" @selected(old('assignment_type') === 'INDIVIDUAL')>ดูแลเฉพาะราย</option>
        </select>
      </label>
      <label>หมายเหตุการมอบหมาย
        <textarea class="input" name="assignment_note" rows="2" placeholder="เช่น ผู้ดูแลประจำตำบล หรือประสานงานเฉพาะเคสนี้">{{ old('assignment_note') }}</textarea>
      </label>
    </div>
  </div>
  ...
  <div class="card" style="margin-top: 16px;">
    ...
    <div class="form-grid plot-grid">
      <label>จังหวัดแปลง<span class="required-star">*</span>
        <select class="input" name="farm_province" id="farm-province-select" data-selected="{{ old('farm_province') }}">
          <option value="">{{ old('farm_province') ? 'กำลังโหลดข้อมูล...' : 'เลือกจังหวัดแปลง' }}</option>
        </select>
      </label>
      <label>ประเภทพืชปลูก<span class="required-star">*</span>
        <select class="input" name="crop_type" id="crop-type-select">
          ...
          @foreach (['ข้าว', 'อ้อย', 'มันสำปะหลัง', 'ข้าวโพด', 'ผัก', 'ผลไม้', 'ยางพารา', 'ปาล์มน้ำมัน', 'อื่นๆ'] as $cropType)
            <option value="{{ $cropType }}" @selected(old('crop_type') === $cropType)>{{ $cropType }}</option>
          @endforeach
        </select>
      </label>
      <label class="plot-area-label">พื้นที่แปลงปลูก<span class="required-star">*</span>
        <div class="area-inputs compact">
          <div class="area-item">
            <input class="input area-field" name="farm_area_rai" type="number" min="0" placeholder="จำนวน" value="{{ old('farm_area_rai') }}">
            <span class="unit">ไร่</span>
          </div>
          <div class="area-item">
            <input class="input area-field" name="farm_area_ngan" type="number" min="0" placeholder="จำนวน" value="{{ old('farm_area_ngan') }}">
            <span class="unit">งาน</span>
          </div>
          <div class="area-item">
            <input class="input area-field" name="farm_area_square_wa" type="number" min="0" placeholder="จำนวน" value="{{ old('farm_area_square_wa') }}">
  ...
  <div class="footer-actions" style="display: none; margin-top: 20px;">
    <button class="btn primary" type="submit" style="min-width:160px; text-align:center;">ถัดไป</button>
  </div>
  <div class="card" style="margin-top: 16px;">
    <h3>กำหนดข้อมูลบัญชีผู้ใช้งาน</h3>
    <div class="form-grid">
      <label>ชื่อผู้ใช้<span class="required-star">*</span>
      ...
      <label>รหัสผ่าน<span class="required-star">*</span>
        <input class="input" name="password" type="password" placeholder="กรอกรหัสผ่าน" autocomplete="new-password">
      </label>
      <label>ยืนยันรหัสผ่าน<span class="required-star">*</span>
        <input class="input" name="password_confirmation" type="password" placeholder="กรอกรหัสผ่านอีกครั้ง" autocomplete="new-password">
      </label>
    </div>
  </div>
  ...
  <div class="footer-actions" style="margin-top: 20px;">
    <button class="btn primary" type="submit" style="min-width:160px; text-align:center;">บันทึกข้อมูล</button>
    <a href="/admin/farmer-users" class="btn ghost">ยกเลิก</a>
  </div>
  @else
  <div class="footer-actions" style="margin-top: 20px;">
    <a href="/admin/farmer-users" class="btn primary" style="min-width:160px; text-align:center;">เสร็จสิ้น</a>
    @if (!empty($createdUserId))
      <a href="/admin/farmer-users/{{ $createdUserId }}/edit" class="btn ghost">แก้ไข</a>
    @else
      <a href="/admin/farmer-users/create" class="btn ghost">แก้ไข</a>
    @endif
  </div>
  @endif
</form>
