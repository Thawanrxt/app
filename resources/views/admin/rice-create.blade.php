...
<form method="POST" action="/admin/rice">
  @csrf
  <div class="card" style="margin-top: 16px;">
    <div class="form-grid">
      <label>ประเภทข้าว
        <input class="input" name="rice_type" type="text" placeholder="กรอกข้อมูล" value="{{ old('rice_type') }}">
      </label>
      <label>ชื่อพันธุ์ข้าว
        <input class="input" name="name" type="text" placeholder="กรอกข้อมูล" value="{{ old('name') }}">
      </label>
      <label>ระยะเวลามาตรฐาน (วัน)
        <input class="input" name="standard_duration_days" type="text" placeholder="จำนวนวัน เช่น 120-150 วัน" value="{{ old('standard_duration_days') }}">
      </label>
      <label>ความต้านทานโรค
        <input class="input" name="disease_resistance" type="text" placeholder="กรอกข้อมูล" value="{{ old('disease_resistance') }}">
      </label>
      <label>
        ความต้านทานศัตรูพืช
        <div class="pest-resistance">
          <div class="inline-field">
            <input class="input" type="text" placeholder="กรอกข้อมูล" id="pest-resistance-input" name="pest_resistances[]" value="{{ old('pest_resistances.0') }}">
            <button class="btn ghost" type="button" id="add-pest-resistance">เพิ่ม</button>
          </div>
          <div class="pest-list" id="pest-list">
            @foreach (collect(old('pest_resistances', []))->skip(1) as $pestResistance)
