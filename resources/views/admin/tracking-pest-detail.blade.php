...
@section('title', 'รายละเอียดการจัดการศัตรูพืช')
@section('content')
@php
  $statusOptions = [
...
<div class="page-head">
  <div class="page-title">
    <a class="back-link icon-only" href="/admin/tracking/pest" aria-label="กลับไปหน้าการจัดการศัตรูพืช">
      <span class="back-icon">‹</span>
    </a>
    <div>
      <h1>รายละเอียดการจัดการศัตรูพืช</h1>
    </div>
  </div>
</div>
...
<div class="detail-layout" style="margin-top:16px;">
  <div class="card">
    ...
    <div class="detail-line"><span class="detail-label">ประเภทศัตรูพืช :</span>{{ $activity->pest_type ?: '-' }}</div>
    <div class="detail-line"><span class="detail-label">ชื่อสามัญสารเคมี :</span>{{ $activity->chemical_name ?: '-' }}</div>
    <div class="detail-line"><span class="detail-label">อัตราส่วนผสม :</span>{{ $activity->mix_ratio ?: '-' }}</div>
    ...
    @if ($activity->image_url)
      <img class="detail-image" src="{{ $activity->image_url }}" alt="รูปการจัดการศัตรูพืช">
    @else
  ...
  <div class="detail-side">
    <div class="card">
      ...
      @endif
      <form method="POST" action="/admin/tracking/pest/{{ $activity->id }}/status" style="margin-top:10px;">
        @csrf
