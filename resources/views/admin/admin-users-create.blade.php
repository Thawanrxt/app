...
<div class="page-head">
  <div class="page-title">
    ...
    <div>
      ...
      <p class="muted">สร้างบัญชีผู้ดูแลระบบและกำหนดขอบเขตการดูแล</p>
    </div>
  </div>
</div>
...
<form method="POST" action="/admin/admin-users" autocomplete="off">
  @csrf
  ...
  <div class="card" style="margin-top: 16px;">
    ...
    <div class="form-grid">
      <label>ชื่อ-สกุล
        <input class="input" name="display_name" type="text" placeholder="กรอกชื่อและนามสกุล" value="{{ old('display_name') }}">
      </label>
      <label>เบอร์โทรศัพท์
        <input class="input" name="phone" type="text" placeholder="กรอกเบอร์โทรศัพท์" value="{{ old('phone') }}">
      </label>
      <label>ตำแหน่งแอดมิน
      ...
      <label>จังหวัดที่รับผิดชอบ
        <select class="input" id="scope_province" name="scope_province" data-selected="{{ old('scope_province') }}">
          <option value="">{{ old('scope_province') ? 'กำลังโหลดข้อมูล...' : 'เลือกจังหวัด' }}</option>
        </select>
      </label>
      <label>อำเภอ/เขตที่รับผิดชอบ
        <select class="input" id="scope_district" name="scope_district" data-selected="{{ old('scope_district', '') }}">
          <option value="">เลือกอำเภอ/เขต</option>
        </select>
      </label>
      <label>ตำบล/แขวงที่รับผิดชอบ
        <input class="input" name="scope_subdistrict" type="text" placeholder="กรอกตำบล/แขวง หรือหลายตำบลคั่นด้วย ," value="{{ old('scope_subdistrict') }}">
      </label>
    </div>
  </div>
      ...
      <label>บทบาท
        <select class="input" name="role">
          @foreach (($roleOptions ?? []) as $roleOption)
            <option value="{{ $roleOption['code'] }}" @selected(old('role', 'ADMIN') === $roleOption['code'])>{{ $roleOption['name_th'] }}</option>
          @endforeach
        </select>
      </label>
      <label>รหัสผ่าน
