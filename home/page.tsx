...
const API_URL = process.env.NEXT_PUBLIC_API_URL || "https://marilou-unsinister-pseudodramatically.ngrok-free.dev";
...
<div className="weather-card" onClick={() => router.push("/weather")} style={{ cursor: "pointer" }}>
  ...
  <div key={plot.id} style={{ backgroundColor: "#f9fff9", border: "1px solid #4CAF50", borderRadius: "10px", padding: "15px", marginBottom: "12px", position: "relative" }}>
    ...
    <button 
      onClick={() => handleDeletePlan(plot.id, plot.plot_name)}
    ...
    <div style={{ marginBottom: "15px" }}>
      ...
      {/* 🆔 แสดง Farm ID ที่ดึงมาจากฐานข้อมูล */}
      <p style={{ fontSize: '11px', color: '#2e7d32', margin: '2px 0 8px 28px', fontWeight: 'bold' }}>
        {plot.farm_id || "FARM-" + plot.id.substring(0,8).toUpperCase()} 
      </p>

      <p style={{ margin: 0, fontSize: "14px", color: "#666", marginLeft: "28px" }}>
        ขนาดพื้นที่: {plot.area_rai || 0} ไร่ {plot.area_ngan || 0} งาน {plot.area_sq_wa || 0} ตร.ว. {plot.area_sq_meter || 0} ตร.ม.
      </p>
    </div>

    <button 
      className="primary-btn" 
      onClick={() => router.push(`/plant-tracking/${plot.id}`)} 
    ...
    >
      📝 บันทึกกิจกรรมแปลงนี้
        ...
        <div className="bottom-nav">
          <div className="nav-item active" onClick={() => router.push("/home")}>🏠<span>หน้าหลัก</span></div>
